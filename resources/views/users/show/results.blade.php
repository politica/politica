    ...
    <div class="items-center justify-between p-6 sm:flex">
        ...
        @if (auth()->check() && user()->id === $user->id && $user->results()->count())
            <div class="items-center flex-shrink-0 hidden px-5 py-3 space-x-3 text-white bg-gray-800 rounded sm:flex">
                ...
                <a href="https://twitter.com/intent/tweet?text={{ urlencode('Check out my political positions on Politica! '.route('users.show', ['user' => $user]).' #IdeologicalResults') }}" target="_blank" class="bg-blue-500 text-white rounded flex-shrink-0 hover:opacity-75 px-2.5 py-1.5 text-xs font-medium space-x-2 leading-4 flex items-center justify-center">
                    <span>Tweet</span>
...
{{--                                <span>{{ $result->value > 50 ? $result->test->label_right : $result->test->label_left }}</span>--}}
{{--                            </h2>--}}
{{--                        </div>--}}
                    </a>
                @endforeach
